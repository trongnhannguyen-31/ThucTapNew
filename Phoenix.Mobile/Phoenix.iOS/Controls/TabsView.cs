...
namespace Naxam.Controls.Platform.iOS
{
    public class TabsView : UIView, IMDTabBarDelegate
    {
        ...
        internal void SetItems2(IEnumerable<string> images)
        {
            _tabBar.SetItems(images.Select(x => (new UIImage(x)).ImageWithRenderingMode(UIImageRenderingMode.AlwaysOriginal)).Cast<NSObject>().ToArray());
        }
